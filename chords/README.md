...
- `charts/pano-se-psili-rachoula.txt`: source lyrics and chords in D, using inline notation such as `[Dm]πάνω`. Blank lines separate verses.
- `songs.mjs`: song metadata and chart location.
...
1. Click **Edit** and change chords in brackets, in the source key D.
...
Tests: `node --test chords/music.test.mjs chords/chart.test.mjs _chords-auth/worker.test.mjs`.
